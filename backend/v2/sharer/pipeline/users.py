...
from .email import get_message, get_misconfiguration_message
...
def send_email(shared_dir: ShareInfo) -> bool:
    ...
    if shared_dir.rdx_share.share_status == ShareStatus.dataset_accepted:
        mail_client = get_dataset_ready_email(shared_dir)
    if shared_dir.rdx_share.share_status == ShareStatus.invalid_permissions:
        ...
        mail_client = get_misconfiguration_email(shared_dir, problem, solution)
    ...
    if shared_dir.rdx_share.share_status == ShareStatus.missing_dataset_config:
        ...
        mail_client = get_misconfiguration_email(
    ...
    # Not every case will require a mail to be sent
    if mail_client:
        mail_client.mail()

    return bool(mail_client)


def get_dataset_ready_email(shared_dir: ShareInfo) -> MailClient:
    ...
    return MailClient(
        ...
        message=get_message(researcher, shared_dir.rdx_share),
    )
